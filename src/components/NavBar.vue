...
<template>
  <nav class="nav">
    <div class="brand">
      <span class="dot"></span>
      <span><span class="accent">alvaro</span><span class="muted">@</span>gomes<span class="muted">:~$</span></span>
    </div>

    <div class="nav-tabs" :class="{ open: menuAberto }">
      <RouterLink to="/" class="tab" exact-active-class="active" @click="closeMenu">
        <span class="tab-num">01</span> <i class="fas fa-terminal"></i> <span>{{ t.nav_home }}</span>
      </RouterLink>
      <RouterLink to="/projetos" class="tab" active-class="active" @click="closeMenu">
        <span class="tab-num">02</span> <i class="fas fa-folder-open"></i> <span>{{ t.nav_projects }}</span>
      </RouterLink>
      <RouterLink to="/experiencia" class="tab" active-class="active" @click="closeMenu">
        <span class="tab-num">03</span> <i class="fas fa-briefcase"></i> <span>{{ t.nav_experience }}</span>
      </RouterLink>
      <RouterLink to="/certificados" class="tab" active-class="active" @click="closeMenu">
        <span class="tab-num">04</span> <i class="fas fa-certificate"></i> <span>{{ t.nav_certs }}</span>
      </RouterLink>
    </div>
...
.brand .accent { color: var(--purple-br); }
.brand .muted { color: var(--faint); }
...
.tab i { font-size: 0.86rem; opacity: 0.85; }
...
.tab .tab-num { color: var(--purple-br); opacity: 0.8; }
...
@media (max-width: 720px) {
    ...
    .nav-tabs.open { transform: translateY(0); }
    .tab { font-size: 0.9rem; padding: 12px 14px; }
